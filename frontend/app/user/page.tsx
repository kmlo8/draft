      ...
      <div className="min-h-screen bg-black text-white">
        ...
        <div className="container mx-auto px-4 pt-24 pb-16">
          <div className="mb-12">
            ...
            {/* DEBUG INFO ON SCREEN (Remove later) */}
            {/* <div className="bg-gray-800 p-4 mb-4 text-xs font-mono text-green-400 whitespace-pre-wrap">
            ...
            <div className="flex items-center gap-6 mb-8">
              ...
              <div>
                <h1 className="text-3xl font-bold mb-2">
                  {user?.name ? `${user.name}님의 마이페이지` : '마이페이지'}
                </h1>
                <p className="text-gray-400">좋아요한 영화, 감독, 배우를 관리하세요</p>
                {/* Fallback display if name is missing but email exists */}
                {!user?.name && user?.email && (
                    <p className="text-xs text-gray-500 mt-1">계정: {user.email}</p>
                )}
              </div>
            </div>
