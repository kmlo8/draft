...
export default function HomePage() {
  ...
  const loadGenreMovies = useCallback(async () => {
    ...
    try {
      ...
      const response = await moviesAPI.getByGenres(preferredGenres);
      const sections = response.data.sections || [];
      
      const koreanSections = sections.map((section: Section) => ({
        ...section,
        title: genreMapToKorean[section.title] || section.title,
        originalTitle: section.title
      }));
      
      const sortedSections = koreanSections.sort((a: Section, b: Section) => {
        const indexA = preferredGenres.indexOf(a.originalTitle as string);
        const indexB = preferredGenres.indexOf(b.originalTitle as string);
  ...
  }, [isAuthenticated, user?.preferredGenres]);
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <section id="search" className="relative min-h-[70vh] flex items-center justify-center">
        ...
        <div className="container mx-auto px-4 py-32 relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            <h1 className="text-5xl md:text-6xl font-bold mb-6">당신을 위한 완벽한 영화를 찾아드립니다</h1>
      ...
      <div className={`pb-16 relative z-10 ${hasSearched ? 'pt-8' : '-mt-20'}`}>
        ...
        {isAuthenticated && genreSections.map(section => (
          <Carousel
            key={`genre-${section.originalTitle}`}
            title={section.title}
        ...
        {!isAuthenticated && !loadingGenreMovies && (
          <div className="container mx-auto px-4 py-8 text-center text-gray-400">
            <p>선호하는 장르를 선택하시면 맞춤형 추천을 해드립니다.</p>
          </div>
        )}
      </div>
    </div>
